...
- **Model rate-limit backoff covers the orchestrator, not yet the fare engine.** Gemini 2.5 Flash runs under Vertex AI's dynamic shared quota, so under regional contention a call can be throttled or rejected with `429 RESOURCE_EXHAUSTED`. The orchestrator's agents now share one model definition with an explicit exponential-backoff-with-jitter retry budget (4 attempts), and when that budget is exhausted the serving layer degrades to a structured, retryable `503 {"error": "model_busy"}` on `/run` instead of a stack trace (see [docs/DECISIONS.md](docs/DECISIONS.md)). The fare engine's inbound LLM does not yet have the equivalent, so that call is the remaining exposed path. Provisioned Throughput would remove the contention entirely but at a fixed monthly cost.
